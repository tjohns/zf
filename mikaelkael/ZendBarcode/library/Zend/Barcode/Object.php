 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Zend_Barcode_Object
{
     ...
     * @var string
     */
    protected $_additionalText = null;
    ...
    public function __construct($options = null)
    {
        ...
        $this->_type = substr(get_class($this), strlen($this->_barcodeNamespace + 1));
    }
    ...
    public function setOrientation($value)
    {
        if (! is_numeric($value)) {
            require_once 'Zend/Barcode/Object/Exception.php';
            throw new Zend_Barcode_Object_Exception('Orientation must be a numeric value');
        }
     ...
     * @return Zend_Barcode_Object
     */
    public function setAdditionalText($value)
    {
        $this->_additionalText = trim($value);
        return $this;
    }

    /**
     * Retrieve additional text to display
     * @return string
     */
     ...
     */
    public function addPolygon($points, $color = null, $filled = true)
    {
        if ($color === null) {
            $color = $this->_foreColor;
        }
        $this->addInstruction(array('type' => 'polygon' , 'points' => $points , 'color' => $color , 'filled' => $filled));
    }

    /**
     * Add a line drawing instruction in the set of instructions
     * @param array $points
     * @param integer $color
     * @param float $thickness
     */
    public function addLine($points, $color = null, $thickness = 1)
    {
        if ($color === null) {
            $color = $this->_foreColor;
        }
        $this->addInstruction(array('type' => 'line' , 'points' => $points , 'color' => $color , 'thickness' => $thickness));
    }
    ...
    protected function _checkRatio($min = 2, $max = 3)
    {
        ...
        if (! ($ratio >= $min && $ratio <= $max)) {
            ...
            throw new Zend_Barcode_Object_Exception(
                sprintf('Ratio thick/thin bar must be between %0.1f and %0.1f (actual %0.3f)',
                    $min,
                    $max,
                    $ratio));
        }
    ...
    /**
     * Calculate the width of a barcode character
     * @return integer
     */
    abstract protected function _characterLength();
    ...
    protected function _calculateBarcodeHeight()
    {
        ...
        if (strlen($this->_additionalText)) {
            $textHeight += $this->_fontSize;
            $extraHeight ++;
        }
    ...
    public function getHeight($recalculate = false)
    {
        if ($this->_height === null || $recalculate) {
            $this->_height = abs($this->_calculateHeight() * cos($this->_orientation / 180 * pi())) + abs(
    ...
    public function getWidth($recalculate = false)
    {
        if ($this->_width === null || $recalculate) {
            $this->_width = abs($this->_calculateWidth() * cos($this->_orientation / 180 * pi())) + abs(
        ...
        if ($this->_offsetLeft === null || $recalculate) {
            $this->_offsetLeft = - min(
                array(
                    ...
                    0 * cos($this->_orientation / 180 * pi()) - $this->_calculateBarcodeHeight() * sin(
                        $this->_orientation / 180 * pi()) ,
            ...
            $this->_offsetTop = - min(
                array(
                    ...
                    $this->_calculateBarcodeHeight() * cos(
                        ...
                        $this->_orientation / 180 * pi()) ,
                    0 * cos($this->_orientation / 180 * pi()) + $this->_calculateBarcodeWidth() * sin(
                        $this->_orientation / 180 * pi())));
        }
    ...
    protected function _drawBarcode()
    {
        ...
        $point1 = $this->_rotate($xpos, $ypos);
        $point2 = $this->_rotate($xpos, $ypos + $this->_calculateHeight());
        $point3 = $this->_rotate($xpos + $this->_calculateWidth(), $ypos + $this->_calculateHeight());
        $point4 = $this->_rotate($xpos + $this->_calculateWidth(), $ypos);
        $this->addPolygon(array($point1 , $point2 , $point3 , $point4), $this->_backgroundColor);
        ...
        foreach ($barcodeTable as $bar) {
            ...
            if ($bar[0]) {
                $point1 = $this->_rotate($xpos, $ypos + $bar[2] * $barLength - 1);
                $point2 = $this->_rotate($xpos, $ypos + $bar[3] * $barLength - 1);
                $point3 = $this->_rotate($xpos + $width - 1, $ypos + $bar[3] * $barLength - 1);
                $point4 = $this->_rotate($xpos + $width - 1, $ypos + $bar[2] * $barLength - 1);
    ...
    protected function _drawBorder()
    {
        if ($this->_withBorder) {
            ...
            $this->addPolygon(array($point1 , $point2 , $point3 , $point4), $this->_foreColor, false);
        }
    }
    ...
    protected function _drawText()
    {
        if ($this->_drawText) {
            if ($this->_stretchText) {
                ...
                $this->addText($this->_text,
                    $this->_fontSize * $this->_factor,
                    $this->_rotate($this->getWidth() / 2,
                        (int) $this->_withBorder * 2 + $this->_factor * ($this->_barHeight + $this->_fontSize)),
                    $this->_font,
                    $this->_foreColor,
                    'center',
                    - $this->_orientation);
    ...
    /**
     * Each children must verify allowed characters
     * @return void
     */
    abstract public function validateText($value);

    /**
     * Each children must prepare the barcode and return
     * a table like array(
     *                      0 => array( 0 => int (visible(black) or not(white))
