...
class Zend_Tool_Provider_ZfProject_ProjectProfile implements IteratorAggregate  
{
    ...
    {
        return (isset($this->_projectContexts[$name]));
    }
    
    public function __get($name)
    {
        return $this->_projectContexts[$name];
    }
    ...
    protected function _serialize()
    {
        ...
        foreach ($this->_projectContexts as $projectContext) {
            self::_serializeRecurser($projectContext, $projectStructureXmlNode);            
        }
    ...
    protected function _serializeRecurser(Zend_Tool_Provider_ZfProject_ProjectContext_ProjectContextAbstract $context, SimpleXmlElement $xmlNode)
    {
        foreach ($context as $subContext) {
            $newNode = $xmlNode->addChild($subContext->getContextName());
            foreach ($subContext->getParameters() as $paramName => $paramValue) {
