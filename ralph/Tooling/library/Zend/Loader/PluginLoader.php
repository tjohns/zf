...
class Zend_Loader_PluginLoader implements Zend_Loader_PluginLoader_Interface 
{
    ...
    /**
     * loadAll() is intended to allow a consumer to load all classes with the specified prefix 
     *
     * @return unknown
     */
    public function loadAll()
    {
        if ($this->_useStaticRegistry) {
            $registry = self::$_staticPrefixToPaths[$this->_useStaticRegistry];
            $loadedPlugins =& self::$_staticLoadedPlugins[$this->_useStaticRegistry];
        } else {
            $registry = $this->_prefixToPaths;
            $loadedPlugins =& $this->_loadedPlugins;
        }
        
        foreach ($registry as $prefix => $paths) {
            foreach ($paths as $path) {
                // make sure path is real or assumed from include_path
                $path = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $path);
                if (($foundPath = realpath($path)) === false) {
                    foreach (explode(PATH_SEPARATOR, get_include_path()) as $includePath) {
                        if (($includePath = realpath($includePath)) === false) {
                            continue;
                        }
                        $includePathPath = $includePath . DIRECTORY_SEPARATOR . $path;
                        if (($foundPath = realpath($includePathPath)) !== false) {
                            break;
                        }
                        $foundPath = false;
                    }
                ...
                foreach(new RecursiveIteratorIterator(new RecursiveDirectoryIterator($foundPath, RecursiveDirectoryIterator::KEY_AS_PATHNAME)) as $fileInfo) {
                    
                    $fileInfoRealPath = $fileInfo->getRealPath();
                    $fileInfoRealPathPathInfo = pathinfo($fileInfoRealPath);

                    if ($fileInfoRealPathPathInfo['extension'] != 'php') {
                        continue;
                    }
                    
                    $classSuffix = str_replace(DIRECTORY_SEPARATOR, '_', substr($fileInfoRealPath, strlen($foundPath)+1, -4));
                    
                    if (!class_exists($prefix . $classSuffix, false)) {
                        include_once $fileInfoRealPath;
                    }
                    
                    $loadedPlugins[$classSuffix] = $prefix . $classSuffix;
                }
                
            }
        }

        return array_values($loadedPlugins);
    }
}
