...
class Zend_Tool_Rpc_Provider_Signature
{
    ...
    public function __construct(Zend_Tool_Rpc_Provider_Interface $provider)
    {
    ...
    protected function _processSpecialties()
    {
        ...
        if ($this->_providerReflection->hasMethod('getSpecialties')) {
            ...
            if (!is_array($specialties)) {
                throw new Zend_Tool_Rpc_Exception('Provider ' . get_class($this->_provider) . ' must return an array for method getSpecialties().');
            }
        } else {
            ...
            if (!is_array($specialties)) {
                throw new Zend_Tool_Rpc_Exception('Provider ' . get_class($this->_provider) . '\'s property $_specialties must be an array.');
            }
        }
