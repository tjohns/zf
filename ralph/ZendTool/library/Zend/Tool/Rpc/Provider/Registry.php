...
class Zend_Tool_Rpc_Provider_Registry implements IteratorAggregate 
{
    ...
    public function addProvider(Zend_Tool_Rpc_Provider_Interface $provider)
    {
    ...
    public function addAction($action)
    {
        if (is_string($action)) {
            $actionName = $action;
            $action = new Zend_Tool_Rpc_Provider_Action();
            $action->setName($actionName);
        }
        
        if (!$action instanceof Zend_Tool_Rpc_Provider_Action) {
            throw new Zend_Tool_Rpc_Provider_Exception('Action must be an instance of Zend_Tool_Rpc_Provider_Action or an action name.');
        }
    ...
    public function process()
    {
        foreach ($this->_providers as $provider) {
            $providerSignature = new Zend_Tool_Rpc_Provider_Signature($provider);
