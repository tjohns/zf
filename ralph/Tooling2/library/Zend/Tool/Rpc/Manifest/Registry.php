...
class Zend_Tool_Rpc_Manifest_Registry //implements IteratorAggregate 
{
     ...
     *
     * @return Zend_Tool_Rpc_Manifest_Registry
     */
        ...
        foreach ($this->_manifests as $manifest) {
            if (method_exists($manifest, 'getMetadata')) {
                ...
                foreach ($metadatas as $metadata) {
                    if (!$metadata instanceof Zend_Tool_Rpc_Manifest_Metadata) {
                        throw new Zend_Tool_Rpc_Manifest_Exception('A Zend_Tool_Rpc_Manifest_Metadata object was not found in manifest ' . get_class($manifest));
                    }
    ...
    protected function _addMetadata(Zend_Tool_Rpc_Manifest_Metadata $metadata)
    {
