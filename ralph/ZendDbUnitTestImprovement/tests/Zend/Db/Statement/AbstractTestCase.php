...
abstract class Zend_Db_Statement_AbstractTestCase extends Zend_Db_TestSuite_AbstractTestCase
{

    public function testStatementConstruct()
    {
        $statementClass = 'Zend_Db_Statement_' . $this->sharedFixture->dbUtility->getDriverName();

        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products');
        $sql = $select->__toString();
        $stmt = new $statementClass($this->sharedFixture->dbAdapter, $sql);
    ...
    public function testStatementConstructWithSelectObject()
    {
        ...
        $select = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products');
        $stmt = new $statementClass($this->sharedFixture->dbAdapter, $select);
        $this->assertType('Zend_Db_Statement_Interface', $stmt);
        $stmt->closeCursor();
    }
    ...
    {
        $stmt = $this->sharedFixture->dbAdapter->select()
            ->from('zf_products')
            ->query();
        $this->assertType('Zend_Db_Statement_Interface', $stmt);
        $stmt->closeCursor();
    ...
    public function testStatementExecuteWithParams()
    {
        ...
        $stmt = $this->sharedFixture->dbAdapter->prepare("INSERT INTO $products ($product_id, $product_name) VALUES (?, ?)");
    ...
    public function testStatementSetFetchModeAssoc()
    {
        ...
        $this->sharedFixture->dbAdapter->setFetchMode(Zend_Db::FETCH_BOTH);

        $stmt = $this->sharedFixture->dbAdapter->query("SELECT * FROM $products WHERE $product_id > 1 ORDER BY $product_id ASC");
    ...
    public function testStatementSetFetchModeBoth()
    {
        ...
        // set the adapter fetch mode to something different
        $this->sharedFixture->dbAdapter->setFetchMode(Zend_Db::FETCH_ASSOC);
