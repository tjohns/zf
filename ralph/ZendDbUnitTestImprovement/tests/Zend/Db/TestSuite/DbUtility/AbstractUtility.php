...
abstract class Zend_Db_TestSuite_DbUtility_AbstractUtility
{
    ...
    /**
     * @var Zend_Db_TestSuite_SQLDialect_Base
     */
    ...
    {
        if ($tableId === null) {
            foreach ($this->_createdResources['tables'] as $tableName) {
                $this->deleteTableDataByTableName($tableName);
            }
        }
        ...
        return $this->deleteTableDataByTableName($this->_createdResources['tables'][$tableId]);
    }
    
    public function deleteTableDataByTableName($tableName, $includeResourcePrefix = false)
    {
        if ($includeResourcePrefix) {
            $tableName = $this->_resourcePrefix . $tableName;
        }
        
        if (!array_search($tableName, $this->_createdResources['tables'])) {
            return;
        }
    ...
    public function resetState()
    {
        ...
        foreach ($this->_createdResources['sequences'] as $sequenceName) {
            public function resetSequence($pr)
        }
        */        
    ...
    public function cleanupResources()
    {
        ...
        foreach ($this->_createdResources['tables'] as $tableId => $tableName) {
            $this->dropTableByName($tableName);
        }
    ...
    public function createTable($tableId, $tableName, array $columns, $includeResourcePrefix = true)
    {
        ...
        $this->dropTableByName($tableName, true);
     ...
     * @param bool $includeResourcePrefix
     */
    public function dropTableByName($tableName, $forceIfUtilityUnknown = false, $includeResourcePrefix = false)
    {
        if ($this->_canManageResources === false) {
            throw new Exception('This utilty cannot create, delete or alter database resources.');
        }
     ...
     * @param bool $includeResourcePrefix
     */
    public function populateDataForTableName($tableName, array $data, $includeResourcePrefix = false)
    {
        if ($includeResourcePrefix) {
            $tableName = $this->_resourcePrefix . $tableName;
        }
    ...
    public function populateData($tableId, array $data)
    {
        ...
        return $this->populateDataForTableName($tableName, $data);
    }
     ...
     * @return string
     */
    public function createProcedure($procedureName, $body, $includeResourcePrefix = false)
    {
        if ($this->_canManageResources === false) {
            throw new Exception('This utilty cannot create, delete or alter database resources.');
        }
     ...
     */
    public function dropProcedure($procedureName, $forceIfUtilityUnknown = false, $includeResourcePrefix = false)
    {
        if ($this->_canManageResources === false) {
            throw new Exception('This utilty cannot create, delete or alter database resources.');
        }
     ...
     * @param bool $includeResourcePrefix
     */
    public function createView($viewName, $asStatement = 'SELECT * FROM', $fromTableName = null, $includeResourcePrefix = false)
    {
        if ($this->_canManageResources === false) {
            throw new Exception('This utilty cannot create, delete or alter database resources.');
        }
     ...
     * @param bool $includeResourcePrefix
     */
    public function dropView($viewName, $forceIfUtilityUnknown = false, $includeResourcePrefix = false)
    {
        if ($this->_canManageResources === false) {
            throw new Exception('This utilty cannot create, delete or alter database resources.');
        }
