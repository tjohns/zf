...
class Zend_Console_Getopt
{
    ...
    const CONFIG_STOPONUNKNOWNOPTION        = 'stopOnUnknownOption';
    ...
    protected $_getoptConfig = array(
        ...
        self::CONFIG_STOPONUNKNOWNOPTION => false
    );
    ...
    /**
     * Stores the current state of whether the parser should continue
     */
    protected $_continueParsing = true;
    ...
    public function parse()
    {
        ...
        while (count($argv) > 0) {
            ...
            if (substr($argv[0], 0, 2) == '--') {
                ...
                if ($this->_getoptConfig[self::CONFIG_STOPONUNKNOWNOPTION]) {
                    $this->_continueParsing = false;
                } else {
                    $this->_remainingArgs[] = array_shift($argv);
                }
            }
            if ($this->_continueParsing == false) {
                $this->_remainingArgs = array_merge($this->_remainingArgs, $argv);
                break;
            }
        }
    ...
    protected function _addRulesModeZend($rules)
    {
        ...
        {
            /**
             * @todo RALPH - allow for dashes in longNamex
             */
            $tokens = preg_split('/([=-])/',
