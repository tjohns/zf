...
class Zend_Config_Xml extends Zend_Config
{
    ...
    protected $_extendsKeyword = 'extends';
    ...
    protected function _processExtends($element, $section, $config = array())
    {
        ...
        if (isset($thisSection[$this->_extendsKeyword])) {
            $extendedSection = (string) $thisSection[$this->_extendsKeyword];
    ...
    {
        $config = array();
        if (count($xmlObject->children()) + count($xmlObject->attributes())) {
            foreach ($xmlObject->attributes() as $key => $value) {
                // Ignore 'extends'
                if ($key == $this->_extendsKeyword) {
                    continue;
                }
                if (array_key_exists($key, $config)) {
                    /** @see Zend_Config_Exception */
                    require_once 'Zend/Config/Exception.php';
                    throw new Zend_Config_Exception("Attribute '$key' already exists.");
                } else {
            ...
            foreach ($xmlObject->children() as $key => $value) {
                ...
                if (isset($config[$key])) {
                    if (!isset($config[$key][0])) {
                    	$valueLast = $config[$key];
                        $config[$key] = array();
                        $config[$key][] = $valueLast;
                    }
                    $config[$key][] = $value;
                } else {
                    $config[$key] = $value;
        ...
        } elseif (!isset($xmlObject[$this->_extendsKeyword])) {
