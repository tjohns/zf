...
abstract class Zend_Pdf_FileParser
{
  /**** Class Constants ****/
     ...
     */
    const BYTEORDER_LITTLEENDIAN = 0;

    /**
     * Big-endian byte order (0x01 0x02 0x03 0x04).
     */
    const BYTEORDER_BIGENDIAN    = 1;
    ...
    public function __construct(Zend_Pdf_FileParserDataSource $dataSource)
    {
        if ($dataSource->getSize() == 0) {
            throw new Zend_Pdf_Exception('The data source has not been properly initialized',
                                         Zend_Pdf_Exception::BADDATASOURCE);
        }
  ...
  /* Parser Methods */
     ...
     * @throws Zend_Pdf_Exception
     */
    public function readInt($size, $byteOrder = Zend_Pdf_FileParser::BYTEORDER_BIGENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
            throw new Zend_Pdf_Exception("Invalid signed integer size: $size",
                                         Zend_Pdf_Exception::INVALIDINTEGERSIZE);
        }
         ...
         */
        if ($byteOrder == Zend_Pdf_FileParser::BYTEORDER_BIGENDIAN) {
            $number = ord($bytes[0]);
            if (($number & 0x80) == 0x80) {
        ...
        } else if ($byteOrder == Zend_Pdf_FileParser::BYTEORDER_LITTLEENDIAN) {
            ...
            throw new Zend_Pdf_Exception("Invalid byte order: $byteOrder",
                                         Zend_Pdf_Exception::INVALIDBYTEORDER);
        }
        return $number;
    }
     ...
     * @throws Zend_Pdf_Exception
     */
    public function readUInt($size, $byteOrder = Zend_Pdf_FileParser::BYTEORDER_BIGENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
                              ...
                              $byteOrder = Zend_Pdf_FileParser::BYTEORDER_BIGENDIAN)
    {
        ...
        if (($bitsToRead % 8) !== 0) {
            throw new Zend_Pdf_Exception('Fixed-point numbers are whole bytes',
                                         Zend_Pdf_Exception::BADFIXEDPOINTSIZE);
        }
