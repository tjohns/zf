   ...
   while ($bucket = stream_bucket_make_writeable($in)) {
     ...
     if($this->mode == self::MODE_ENCODE) {
       if($bucket->datalen>0) {
         ...
         // cut the trailing characters to that the remainder is 
         // dividable by 3 if it is not.
         if($trailing>0) {
         ...
         // now cut the last line off, because this might not be
         // LINEEND characters. We prepend this to the next chunk...
         $p = strrpos($chunk, "\n"); // cut off last line
         if($p!==false) {
         ...
         if($this->lastline.$this->backlog=="") return PSFS_FLAG_FLUSH_CLOSE;
