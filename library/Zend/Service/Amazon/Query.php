...
class Zend_Service_Amazon_Query extends Zend_Service_Amazon
{
	...
	function __call($method, $args)
	{
		...
		} else if ($this->_searchIndex !== null || $this->_searchIndex == 'asin') {
			if (in_array($method, self::$_searchParams[$this->_searchIndex])) {
				$this->_search[$method] = $args[0];
			} else {
				throw new Zend_Service_Exception("Unknown search parameter for \"$this->_searchIndex\" search");
			}
		} else {
	...
	function search()
	{
		if ($this->_searchIndex['idType'] == 'asin') {
