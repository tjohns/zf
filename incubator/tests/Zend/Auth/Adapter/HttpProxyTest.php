 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Auth_Adapter_HttpProxyTest extends PHPUnit_Framework_TestCase
{
     ...
     *
     * @param string Authenticate header value
     * @param string Which authentication scheme to use
     * @return array Containing the result, the response headers, and the 
     * status
     * @throws none
     */
    public function _doAuth($clientHeader, $scheme)
    {
        ...
        switch ($scheme) {
            case 'basic':
                $use = $this->_basicConfig; break;
            case 'digest':
                $use = $this->_digestConfig; break;
            case 'both':
            default:
                $use = $this->_bothConfig; break;
        }
    ...
    protected function _digestReply($user, $pass)
    {
        ...
        $opaque = md5('Opaque Data:Zend_Auth_Adapter_Http');
        $cnonce = md5('cnonce');
        $response = md5(md5($user.':'.$this->_digestConfig['realm'].':'.$pass).":$nonce:$nc:$cnonce:auth:".md5('GET:/'));
        $cauth = 'Digest '
               . 'username="Bryce", '
               . 'realm="'.$this->_digestConfig['realm'].'", '
               . 'nonce="'.$nonce.'", '
     ...
     *
     * @param array Authentication results
     * @param string Expected Proxy-Authenticate header value
     */
