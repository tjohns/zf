 ...
 * @subpackage UnitTests
 */
class Zend_Measure_BinaryTest extends PHPUnit2_Framework_TestCase
{
    ...
    public function testBinarySerialize()
    {
        ...
        $this->assertTrue(!empty($serial),'Zend_Measure_Binary not serialized');
    }
     ...
     * expected object
     */
    public function testBinaryUnSerialize()
    {
        $value = new Zend_Measure_Binary('string -100.100,200',Zend_Measure_Binary::STANDARD,'de');
        $serial = $value->serialize();
        $newvalue = unserialize($serial);
        $this->assertTrue($value->equals($newvalue),'Zend_Measure_Binary not unserialized');
    }
