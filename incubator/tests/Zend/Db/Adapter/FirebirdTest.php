...
class Zend_Db_Adapter_FirebirdTest extends Zend_Db_Adapter_TestCommon
{
    ...
    {
        $this->markTestSkipped('Firebird does not support auto-increment');
    }
    ...
    public function testAdapterDescribeTablePrimaryKeyColumn()
    {
        ...
        // Firebird reports precsion 11 for integers
    ...
    public function testAdapterFetchAll()
    {
        ...
        $result = $this->_db->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", array(1));
    ...
    public function testAdapterFetchAssoc()
    {
        ...
        $result = $this->_db->fetchAssoc("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id DESC", array(1));
    ...
    public function testAdapterFetchCol()
    {
        ...
        $result = $this->_db->fetchCol("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", array(1));
    ...
    public function testAdapterFetchOne()
    {
        ...
        $prod = 'Linux';
        $result = $this->_db->fetchOne("SELECT $product_name FROM $products WHERE $product_id > ? ORDER BY $product_id", array(1));
    ...
    public function testAdapterFetchPairs()
    {
        ...
        $prod = 'Linux';
        $result = $this->_db->fetchPairs("SELECT $product_id, $product_name FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", array(1));
        $this->assertEquals(2, count($result)); // count rows
    ...
    public function testAdapterFetchRow()
    {
        ...
        $result = $this->_db->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", array(1));
     ...
     * Firebird does not want the 'AS' in between.
     */
