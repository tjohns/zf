...
abstract class Zend_Translate_Adapter {
     ...
     * @return Zend_Translate_Adapter         Fluid Interface
    ...
    public function translate($messageId, $messagePlural = null, $plural = null, $locale = null)
    {
        ...
        if (is_int($plural) and ($plural > 0)) {
            ...
            if ((array_key_exists($locale, $this->_translate)) and 
                (array_key_exists($messageId, $this->_translate[$locale])) and
                (array_key_exists($plural, $this->_translate[$locale][$messageId]))) {
                // return original translation
                return $this->_translate[$locale][$messageId][$plural];
            } else if (strlen($locale) > 3) {
     ...
     * @return string
     */
    public function _($messageId, $plural = null, $locale = null)
    {
        return $this->translate($messageId, $plural, $locale);
    }
    ...
    public function isTranslated($messageId, $original = false, $locale = null)
    {
        ...
        if ((array_key_exists($locale, $this->_translate)) and
            ...
            // return original translation
            return true;
        } else if ((strlen($locale) > 3) and ($original === false)) {
            // faster than creating a new locale and separate the leading part
            $locale = substr($locale, 0, -strlen(strrchr($locale, '_')));
