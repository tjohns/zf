...
require_once 'Zend/Tool/CodeGenerator/Php/Docblock/Tag.php';

class Zend_Tool_CodeGenerator_Php_Docblock extends Zend_Tool_CodeGenerator_Php_Abstract
{
    ...
    public static function fromReflection(Zend_Reflection_Docblock $reflectionDocblock)
    {
        ...
        foreach ($reflectionDocblock->getTags() as $tag) {
            $docblock->setTag(Zend_Tool_CodeGenerator_Php_Docblock_Tag::fromReflection($tag));
        }
    ...
    {
        if (is_array($tag)) {
            $tag = new Zend_Tool_CodeGenerator_Php_Docblock_Tag($tag);
        } elseif ($tag instanceof Zend_Tool_CodeGenerator_Php_Docblock_Tag) {
            // anything here?
        } else {
            require_once 'Zend/Tool/CodeGenerator/Php/Exception.php';
            throw new Zend_Tool_CodeGenerator_Php_Exception('setTag() expects either an array of method options or an instance of Zend_Tool_CodeGenerator_Php_Docblock_Tag');
        }
