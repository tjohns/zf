...
require_once 'Zend/Tool/CodeGenerator/Php/Class.php';

class Zend_Tool_CodeGenerator_Php_File extends Zend_Tool_CodeGenerator_Php_Abstract
{
    
    protected static $_markerDocblock = '/* Zend_Tool_CodeGenerator_Php_File-DocblockMarker */';
    protected static $_markerRequire = '/* Zend_Tool_CodeGenerator_Php_File-RequireMarker: {?} */';
    protected static $_markerClass = '/* Zend_Tool_CodeGenerator_Php_File-ClassMarker: {?} */';
     ...
     * @return Zend_Tool_CodeGenerator_Php_File
     */
    public static function fromReflection(Zend_Reflection_File $reflectionFile)
    {
        ...
        foreach ($reflectionFile->getClasses() as $class) {
            $file->setClass(Zend_Tool_CodeGenerator_Php_Class::fromReflection($class));
            ...
            for ($lineNum = 1; $lineNum <= count($bodyLines); $lineNum++) {
                if ($lineNum == $classStartLine) { 
                    $bodyReturn[] = str_replace('?', $class->getName(), self::$_markerClass);  //'/* Zend_Tool_CodeGenerator_Php_File-ClassMarker: {' . $class->getName() . '} */';
        ...
        if ($docblock = $reflectionFile->getDocblock()) {
            $file->setDocblock(Zend_Tool_CodeGenerator_Php_Docblock::fromReflection($docblock));
            
            $bodyLines = explode("\n", $body);
            $bodyReturn = array();
            for ($lineNum = 1; $lineNum <= count($bodyLines); $lineNum++) {
                if ($lineNum == $docblock->getStartLine()) { 
                    $bodyReturn[] = str_replace('?', $class->getName(), self::$_markerDocblock);  //'/* Zend_Tool_CodeGenerator_Php_File-ClassMarker: {' . $class->getName() . '} */';
     ...
     * Set the docblock
     *
     * @param Zend_Tool_CodeGenerator_Php_Docblock $docblock
     * @return Zend_Tool_CodeGenerator_Php_File
     */
     ...
     * Get docblock
     *
     * @return Zend_Tool_CodeGenerator_Php_Docblock
     */
    ...
    {
        if (is_array($class)) {
            $class = new Zend_Tool_CodeGenerator_Php_Class($class);
            $className = $class->getName();
        } elseif ($class instanceof Zend_Tool_CodeGenerator_Php_Class) {
            $className = $class->getName();
        } else {
            require_once 'Zend/Tool/CodeGenerator/Php/Exception.php';
            throw new Zend_Tool_CodeGenerator_Php_Exception('Expecting either an array or an instance of Zend_Tool_CodeGenerator_Php_Class');
        }
     ...
     * Enter description here...
     *
     * @return Zend_Tool_CodeGenerator_Php_Class[]
     */
