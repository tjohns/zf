...
class Zend_Tool_Framework_Client_Cli_ArgumentParser
{
    ...
    public function __construct(Array $arguments)
    {
        ...
        $this->_manifestRepository = $clientRegistry->getManifestRepository();
        $this->_clientRequest      = $clientRegistry->getRequest();
        $this->_clientResponse     = $clientRegistry->getResponse();
        
        if ($this->_clientRequest == null || $this->_clientResponse == null) {
    ...
    protected function _parseGlobalPart()
    {
        ...
        foreach ($getoptParser->getOptions() as $option) {
            if ($option == 'pretend') {
                $this->_clientRequest->setPretend(true);
            } else {
                $property = '_'.$option;
                $this->{$property} = true;                
            }
    ...
    protected function _parseProviderPart()
    {
        ...
        if ($cliProviderName == '?' || $cliSpecialtyName == '?') {
            $this->_help = true;
            $this->_clientRequest->setProviderName($cliProviderName);
            $this->_clientRequest->setSpecialtyName($cliSpecialtyName);
            return;
        }
    ...
    protected function _createHelpResponse(Array $options = array())
    {
        ...
        // provider name is known, action is not
            ...
            foreach ($providerMetadatas as $providerMetadata) {
                ...
                //$response .= "    \033[32m(" . implode('|', $responseProviderActions) . ")\033[37m " . $providerMetadata->getValue() . PHP_EOL;
