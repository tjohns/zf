 ...
 *
 */
class Zend_Tool_Framework_Endpoint_Cli_ArgumentParser
{
     ...
     * @param Zend_Tool_Framework_Endpoint_Request $endpointRequest
     * @param Zend_Tool_Framework_Endpoint_Response $endpointResponse
     */
    public function __construct(Array $arguments, Zend_Tool_Framework_Endpoint_Request $endpointRequest, Zend_Tool_Framework_Endpoint_Response $endpointResponse)
    {
        // set the arguments
    ...
    protected function _parseActionPart()
    {
        ...
        $cliActionMetadata = Zend_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type' => 'Action', 'name' => 'cliActionName', 'value' => $cliActionName));
        
        // if no action, handle error
        if (!$cliActionMetadata) {
            require_once 'Zend/Tool/Framework/Endpoint/Exception.php';
            throw new Zend_Tool_Framework_Endpoint_Exception('Action \'' . $cliActionName . '\' is not a valid action.');
        }
    ...
    protected function _parseProviderPart()
    {
        ...
        $cliProviderMetadata = Zend_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type'=>'Provider', 'name' => 'cliProviderName', 'value' => $cliProviderName));

        if (!$cliProviderMetadata) {
            require_once 'Zend/Tool/Framework/Endpoint/Exception.php';
            throw new Zend_Tool_Framework_Endpoint_Exception('Provider \'' . $cliProviderFull . '\' is not a valid provider.');
        }
        
        $cliSpecialtyMetadata = Zend_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type' => 'Provider', 'name' => 'cliSpecialtyNames', 'providerName' => $cliProviderMetadata->getProviderName(), 'value' => $cliSpecialtyName));
        
        if (!$cliSpecialtyMetadata) {
            require_once 'Zend/Tool/Framework/Endpoint/Exception.php';
            throw new Zend_Tool_Framework_Endpoint_Exception('Provider \'' . $cliSpecialtyName . '\' is not a valid specialty.');
        }
    ...
    protected function _createHelpResponse(Array $options = array())
    {
        ...
        if (($actionName = $this->_endpointRequest->getActionName()) && ($providerName = $this->_endpointRequest->getProviderName())) {
        	...
        	$providerMetadatas = Zend_Tool_Framework_Manifest_Registry::getInstance()->getMetadatas(array('type'=>'Provider', 'name' => 'cliProviderName'));
	        ...
	        foreach ($providerMetadatas as $providerMetadata) {
	        	...
	        	foreach ($providerMetadata->getReference()->getActionableMethods() as $actionableMethod) {
	        		...
	        		$actionMetadata = Zend_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type'=>'Action', 'name' => 'cliActionName', 'actionName' => $actionName));
