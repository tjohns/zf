    ...
    public function create($name, $indexActionIncluded = true)
    {
        ...
        if ($this->_getRequest()->isPretend()) {
            ...
            if (isset($indexActionResource)) {
                $this->_getResponse()->appendContent('Would create an index action method in controller ' . $name);
                $this->_getResponse()->appendContent('Would create a view script for the index action method at ' . $indexActionViewResource->getContext()->getPath());
            }
            
            if ($testControllerResource) {
                $this->_getResponse()->appendContent('Would create a controller test file at ' . $testControllerResource->getContext()->getPath());
            }
            ...
            if (isset($indexActionResource)) {
                $this->_getResponse()->appendContent('Creating an index action method in controller ' . $name);
                $indexActionResource->create();
                $this->_getResponse()->appendContent('Creating a view script for the index action method at ' . $indexActionViewResource->getContext()->getPath());
                $indexActionViewResource->create();
            }
            
            if ($testControllerResource) {
                $this->_getResponse()->appendContent('Creating a controller test file at ' . $testControllerResource->getContext()->getPath());
