 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */ 
class Zend_Log_Writer_File extends Zend_Log_Writer_Abstract
{
    /**
     ...
     * @var      array
     */
    protected $_options = array('buffer'      => false,
                                'bufferLines' => 20,
                                'keepOpen'    => false,
	...
	public function open($filename = null, $accessMode = null)
	{
        ...
        if (!$this->_fileResource = @fopen($this->_filename, $this->_accessMode, false)) {
            throw new Zend_Log_Exception("Log file \"$filename\" could not be opened");
        }
    ...
    public function write($message, $level)
    {
	    ...
	    $this->_buffer[] = $this->_parseLogLine($message, $level);
	    ++$this->_bufferedLines;
	...
	public function flush()
	{
	    ...
	    foreach ($this->_buffer as $line) {
	        if (!fwrite($this->_fileResource, $line . $this->_lineEnding)) {
	            throw new Zend_Log_Exception("Log file \"{$this->_filename}\" could not be written.");
	        }
	    }
	 ...
	 * @return string
	 */
	protected function _parseLogLine($message, $level)
	{
        $fields = array('message' => $message, 'level' => $level);
    ...
    protected function _setAccessMode($accessMode)
    {
        ...
        if ($accessMode != 'w' && $accessMode != 'a') {
            throw new Zend_Log_Exception("Illegal access mode specified.  Specify 'a' for append or 'w' for overwrite.");
        }
