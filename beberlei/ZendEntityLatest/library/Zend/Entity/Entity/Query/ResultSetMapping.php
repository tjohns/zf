...
class Zend_Entity_Query_ResultSetMapping
{
    ...
    public function addEntity($entityName, $alias=null)
    {
        ...
        $this->rootEntity[$alias] = $entityName;
        $this->aliasToEntity[$alias] = $entityName;
        $this->entityResult[$alias] = array(
            'properties' => array(),
        );
        return $this;
    }
     ...
     * @return Zend_Entity_Query_ResultSetMapping
     */
    public function addJoinedEntity($entityName, $alias=null, $parentAlias=null, $parentProperty=null)
    {
        if($alias == null) {
            $alias = $entityName;
        }

        $this->joinedEntity[$alias] = array('parentAlias' => $parentAlias, 'parentProperty' => $parentProperty);
        $this->aliasToEntity[$alias] = $entityName;
        $this->entityResult[$alias] = array(
            'properties' => array(),
        );
    ...
    {
        if(!isset($this->entityResult[$alias])) {
            throw new Zend_Entity_Exception(
                "No valid alias or entity name '".$alias."' was given to add property '".$propertyName."' to."
            );
        }
        $this->entityResult[$alias]['properties'][$storageFieldName] = $propertyName;
        $this->storageFieldEntity[$storageFieldName] = $alias;
        return $this;
    }
