...
class Zend_Entity_ManagerTest extends Zend_Entity_TestCase
{
    ...
    public function testDetach_OnNonObject_ThrowsException()
    {
    ...
    public function testCloseConnection_DelegatesToAdapter()
    {
    ...
    public function testGetReference_InitializesProxies_ButOnlyOnce()
    {
        $em = $this->createTestingEntityManager();
        $proxyGeneratorMock = $this->getMock('Zend_Entity_LazyLoad_GeneratorAbstract');
        $proxyGeneratorMock->expects($this->once())
                           ->method('generate');
        $proxyGeneratorMock->expects($this->exactly(3))
                           ->method('instantiate')
                           ->will($this->returnValue(new stdClass));
        $metadataFactory = $this->getMock('Zend_Entity_MetadataFactory_FactoryAbstract');
        $metadataFactory->expects($this->once())
                        ->method('visit')
                        ->with($this->equalTo($proxyGeneratorMock));
        $em->setProxyGenerator($proxyGeneratorMock);
        $em->setMetadataFactory($metadataFactory);

        $proxyA = $em->getReference("Zend_TestEntity1", 1);
        $proxyB = $em->getReference("Zend_TestEntity1", 2);
        $proxyC = $em->getReference("Zend_TestEntity1", 3);
    }
    ...
    public function testGetReference_CreatesLazyLoad_IfObjectNotLoadedBefore()
    {
        ...
        $proxyGeneratorMock->expects($this->once())
                           ->method('instantiate')
                           ->with($this->equalTo($fixtureClass), $this->equalTo($manager))
                           ->will($this->returnValue($entity));
        
        $manager->setProxyGenerator($proxyGeneratorMock);

        $actual = $manager->getReference($fixtureClass, $fixtureId);

        $this->assertType('stdClass', $actual);
    ...
    public function testMerge_NotYetImplemented()
    {
        $this->setExpectedException("Zend_Entity_Exception", "not implemented yet");

        $em = $this->createEntityManager();
        $em->merge(new Zend_TestEntity1);
    }

    public function testRefresh_DelegatesToMapper()
    {
        $refreshEntity = new Zend_TestEntity1;

        $em = $this->createEntityManager();

        $mapperMock = $this->getMock('Zend_Db_Mapper_Mapper', array(), array(), '', false);
        $mapperMock->expects($this->once())
                   ->method('refresh')
                   ->with($this->equalTo($refreshEntity, $em));

        $em->setMapper($mapperMock);
        $em->refresh($refreshEntity);
    }
}
