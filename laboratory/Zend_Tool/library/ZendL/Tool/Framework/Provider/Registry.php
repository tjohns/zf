...
class ZendL_Tool_Framework_Provider_Registry implements IteratorAggregate 
{
     ...
     *
     * @return ZendL_Tool_Framework_Provider_Registry
     */
    ...
    public function addProvider(ZendL_Tool_Framework_Provider_Interface $provider)
    {
    ...
    public function addAction($action)
    {
        if (is_string($action)) {
            $actionName = $action;
            $action = new ZendL_Tool_Framework_Provider_Action();
            $action->setName($actionName);
        }
        
        if (!$action instanceof ZendL_Tool_Framework_Provider_Action) {
            throw new ZendL_Tool_Framework_Provider_Exception('Action must be an instance of ZendL_Tool_Framework_Provider_Action or an action name.');
        }
    ...
    public function process()
    {
        foreach ($this->_providers as $provider) {
            $providerSignature = new ZendL_Tool_Framework_Provider_Signature($provider);
