...
class ZendL_Tool_Framework_Provider_Signature
{
    ...
    public function __construct(ZendL_Tool_Framework_Provider_Interface $provider)
    {
    ...
    protected function _processSpecialties()
    {
        ...
        if ($this->_providerReflection->hasMethod('getSpecialties')) {
            ...
            if (!is_array($specialties)) {
                throw new ZendL_Tool_Framework_Exception('Provider ' . get_class($this->_provider) . ' must return an array for method getSpecialties().');
            }
        } else {
            ...
            if (!is_array($specialties)) {
                throw new ZendL_Tool_Framework_Exception('Provider ' . get_class($this->_provider) . '\'s property $_specialties must be an array.');
            }
        }
    ...
    protected function _processActionableMethods()
    {
        ...
        foreach ($methods as $method) {
            ...
            $actionableMethods[$methodName]['action'] = ZendL_Tool_Framework_Provider_Registry::getInstance()->getAction($actionableMethods[$methodName]['actionName']);
