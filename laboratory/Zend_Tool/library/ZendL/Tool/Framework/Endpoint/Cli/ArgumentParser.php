 ...
 *
 */
class ZendL_Tool_Framework_Endpoint_Cli_ArgumentParser
{
    
    /**
     * @var ZendL_Tool_Framework_Endpoint_Request
     */
    protected $_endpointRequest = null;
    
    /**
     * @var ZendL_Tool_Framework_Endpoint_Response
     */
    protected $_endpointResponse = null;
    ...
    /**
     * Constructor, takes in an endpoint request and response, as well as the arguments.
     *
     * @param array $arguments
     * @param ZendL_Tool_Framework_Endpoint_Request $endpointRequest
     * @param ZendL_Tool_Framework_Endpoint_Response $endpointResponse
     */
    public function __construct(Array $arguments, ZendL_Tool_Framework_Endpoint_Request $endpointRequest, ZendL_Tool_Framework_Endpoint_Response $endpointResponse)
    {
        // set the arguments
        $this->_argumentsOriginal = $this->_argumentsWorking = $arguments;
        $this->_endpointRequest   = $endpointRequest;
        $this->_endpointResponse  = $endpointResponse;
    }
    ...
    public function parse()
    {
        ...
        if (count($this->_argumentsWorking) == 0) {
            $this->_endpointRequest->setDispatchable(false); // at this point request is not dispatchable
            if ($this->_help) {
    ...
    protected function _parseGlobalPart()
    {
    	...
    	$getoptParser = new ZendL_Console_Getopt($getoptOptions, $this->_argumentsWorking, array('parseAll' => false));
    	$getoptParser->parse();
    ...
    protected function _parseActionPart()
    {
        ...
        $cliActionMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type' => 'Action', 'name' => 'cliActionName', 'value' => $cliActionName));
        
        // if no action, handle error
        if (!$cliActionMetadata) {
            require_once 'ZendL/Tool/Framework/Endpoint/Exception.php';
            throw new ZendL_Tool_Framework_Endpoint_Exception('Action \'' . $cliActionName . '\' is not a valid action.');
        }
        
        // prepare action request name
        $this->_endpointRequest->setActionName($cliActionMetadata->getActionName());
    ...
    protected function _parseProviderPart()
    {
        ...
        $cliProviderMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type'=>'Provider', 'name' => 'cliProviderName', 'value' => $cliProviderName));

        if (!$cliProviderMetadata) {
            require_once 'ZendL/Tool/Framework/Endpoint/Exception.php';
            throw new ZendL_Tool_Framework_Endpoint_Exception('Provider \'' . $cliProviderFull . '\' is not a valid provider.');
        }
        
        $cliSpecialtyMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type' => 'Provider', 'name' => 'cliSpecialtyNames', 'providerName' => $cliProviderMetadata->getProviderName(), 'value' => $cliSpecialtyName));
        
        if (!$cliSpecialtyMetadata) {
            require_once 'ZendL/Tool/Framework/Endpoint/Exception.php';
            throw new ZendL_Tool_Framework_Endpoint_Exception('Provider \'' . $cliSpecialtyName . '\' is not a valid specialty.');
        }
        
        // prepare provider request name
        $this->_endpointRequest->setProviderName($cliProviderMetadata->getProviderName());
        $this->_metadataProvider = $cliProviderMetadata;
        $this->_endpointRequest->setSpecialtyName($cliSpecialtyMetadata->getSpecialtyName());
    ...
    protected function _parseProviderOptionsPart()
    {
    	$searchParams = array(
            'type'=>'Provider', 
            'providerName'  => $this->_endpointRequest->getProviderName(), 
            'actionName'    => $this->_endpointRequest->getActionName(), 
            'specialtyName' => $this->_endpointRequest->getSpecialtyName(), 
            );
    	
        $cliActionableMethodLongParameterMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(
            array_merge($searchParams, array('name' => 'cliActionableMethodLongParameters'))
            );

        $cliActionableMethodShortParameterMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(
        ...
        if ($wordStack && $wordArguments) {
            for ($wordIndex = 1; $wordIndex <= count($wordArguments); $wordIndex++) {
                ...
                $this->_endpointRequest->setProviderParameter($wordArguments[$wordIndex]['parameterName'], $wordStack[$wordIndex-1]);
        ...
        foreach ($getoptParser->getOptions() as $option) {
            ...
            $this->_endpointRequest->setProviderParameter($option, $value);
        }
    ...
    protected function _createHelpResponse(Array $options = array())
    {
        ...
        if (($actionName = $this->_endpointRequest->getActionName()) && ($providerName = $this->_endpointRequest->getProviderName())) {
        	...
        	$providerMetadatas = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadatas(array('type'=>'Provider', 'name' => 'cliProviderName'));
	        ...
	        foreach ($providerMetadatas as $providerMetadata) {
	        	...
	        	foreach ($providerMetadata->getReference()->getActionableMethods() as $actionableMethod) {
	        		...
	        		$actionMetadata = ZendL_Tool_Framework_Manifest_Registry::getInstance()->getMetadata(array('type'=>'Action', 'name' => 'cliActionName', 'actionName' => $actionName));
        ...
        $this->_endpointResponse->appendContent($response);
        return;
    }
